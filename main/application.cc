...
void Application::mpu6050(void *pvParameters)
{
    ...
    while (1)
    {
        if (gpio_get_level(BUTTON_GPIO) == 0)
        {
                ...
                if (mpu.dmpGetCurrentFIFOPacket(fifoBuffer))
                {
                    ...
                    bool should_continue = false;
                    {
                        ...
                        if (is_collecting && current_index < app.kMaxCollectedTimeSteps)
                        {
                            ...
                            app.collected_data[current_index * app.kNumFeaturesPerStep + 0] = (float)ax;
                            app.collected_data[current_index * app.kNumFeaturesPerStep + 1] = (float)ay;
                            app.collected_data[current_index * app.kNumFeaturesPerStep + 2] = (float)az;
                            app.collected_data[current_index * app.kNumFeaturesPerStep + 3] = (float)gx;
                            app.collected_data[current_index * app.kNumFeaturesPerStep + 4] = (float)gy;
                            app.collected_data[current_index * app.kNumFeaturesPerStep + 5] = (float)gz;

                            app.collected_data_index = current_index + 1; // 原子更新索引

                            // 如果已收集足够的数据，停止收集
                            if (current_index + 1 >= app.kMaxCollectedTimeSteps)
                            {
            ...
            {
                ESP_LOGW(TAG, "data not enough for inference, collected %d samples, resetting...", final_collected_count);
            }
            // 无论是否进行推理，都重置数据收集索引
            app.collected_data_index = 0;
        }
